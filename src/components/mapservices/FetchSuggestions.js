...
const FetchSuggestions = ({ query, region, onSelect }) => {
  ...
  useEffect(() => {
    if (query !== "" && query.length > 2) {
      fetch('https://carpool-backend-079r.onrender.com/generate-token')
        .then(response => response.json())
        .then(data => {
          fetch('https://carpool-backend-079r.onrender.com/getToken')
            .then(response => response.json())
            .then(data => {
              fetch(`https://carpool-backend-079r.onrender.com/autosuggest?query=${query}&region=${region}&token=${data.token.access_token}`, {
                headers: {
